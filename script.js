...
// Donation bar functionality
const donationBar = document.getElementById('donationBar');
const container = document.querySelector('.container');

function closeDonationBar() {
    donationBar.style.display = 'none';
    sessionStorage.setItem('donationBarClosed', 'true');
    container.style.marginTop = '0px';  // Adjust margin when donation bar is closed
}

document.addEventListener('DOMContentLoaded', function () {
    if (sessionStorage.getItem('donationBarClosed') === 'true') {
        donationBar.style.display = 'none';
        container.style.marginTop = '0px';  // Ensure margin is updated on page load
    } else {
        container.style.marginTop = '50px';  // Adjust based on donation bar height
    }
});
